	...
	<div class="container">
		<div class="banner_text">
			...
			<ul>
				...
				<li><a href="cart.jsp">Đơn hàng</a></li>
			</ul>
		</div>
	</div>
</section>
			...
			<table class="table">
				<thead>
					<tr>
						...
						<th scope="col" style="width : 190px;">Trạng Thái</th>
						<th scope="col">Tổng</th>
						<th scope="col">Hành động</th>
					</tr>
				</thead>
				<tbody>
					<c:set var="cart" value="${orders}" />
					<c:choose>
						<c:when test="${empty cart}">
							<tr>
								<td colspan="6" style="text-align: center;">Đơn hàng của
									bạn đang trống.</td>
							</tr>
						</c:when>
						<c:otherwise>
							...
							<c:forEach var="item" items="${cart}">
								<tr>
									<td>${item.id}</td>
									<td><fmt:formatDate value = "${item.createdAt}" pattern=" HH:mm 'ngày' dd/MM/yyyy"/></td>
									<td><c:choose>
											<c:when test="${item.status == 'PENDING'}">
												<span >Chờ xử lý</span>
											</c:when>
											<c:when
												test="${item.status == 'PROCESSING' || item.status == 'CONFIRMED'}">
												<span >Đang xử lý</span>
											</c:when>
											<c:when test="${item.status == 'PAID'}">
												<span >Đã thanh toán</span>
											</c:when>
											<c:when test="${item.status == 'DELIVERING'}">
									...
									<td><fmt:formatNumber value="${item.totalAmount}"
											pattern="#,##0" />₫</td>
									<td><a class="pest_btn add-to-cart-btn"
										href="DetaillistuserOrders?id=${item.id}">Xem</a></td>
								</tr>
							</c:forEach>
						</c:otherwise>
					</c:choose>
