...
@media ( max-width : 768px) {
	...
	<section class="banner_area">
		<div class="container">
			<div class="banner_text">
				<h3>Sản phẩm của Nõn Café</h3>
				<ul>
					<li><a href="home">Home</a></li>
					<li><a href="CakeControl">Sản phẩm</a></li>
				</ul>
			</div>
		</div>
	</section>
	...
	<section class="portfolio_area portfolio_full p_100">
		<div class="portfolio_filter">
			<ul class="list_style">
				<c:forEach var="c" items="${listC}" varStatus="status">
					<li class="${status.first ? 'active' : ''}" data-filter="*"><a
						href="javascript:void(0);"
						onclick="window.location.href='ProductCategoriesKM?id=${c.id}'">
							${c.name} </a></li>
				</c:forEach>
			</ul>
		</div>
		...
		<div
			class="portfolio_full_width_area grid_portfolio_area imageGallery1">
			<c:forEach var="p" items="${listP}">
				<div class="portfolio_full_item cake choco">
					<div class="portfolio_item">
						<div class="portfolio_img">
							<a class="light" href="${p.imageUrl}"> <img class="img-fluid"
								src="${p.imageUrl}" alt="${p.name}">
							</a>
						</div>
						<div class="portfolio_text">
							<a href="detail?pid=${p.id}&categoryId=${p.categoryId}"><h4>${p.name}</h4></a>
						</div>
					</div>
				</div>
			</c:forEach>
	...
	<script>
		jQuery(document).ready(function($) {
			// Vô hiệu hóa Isotope filtering cho portfolio_filter
			$('.portfolio_filter ul li a').off('click');

			// Xóa active class khi click
